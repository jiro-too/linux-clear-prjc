...
extern FILE *access_file(char *file, const char *perms, int code);
...
void _vprintf(char *fmt, ...)
{
    ...
    if (_v_flag)
    {
        fprintf(stdout, "%s", VERBOSE_PREMSG);
 ...
 *
 */
// TODO: Namespace remove space function
...
g_kver()
{
    ...
    char config_to_read[2048];
    sprintf(config_to_read, "%s/Makefile", kernel_path);
    _vprintf("Reading file \"%s\"\n",config_to_read);
    FILE *Makefile = access_file(config_to_read, "r", HARD_EXIST);

    // TODO: Check for EXTRAVERISON
    char VERSION[100];
    char PATCHLEVEL[100];
    char SUBLEVEL[100];

    while (getline(&buf,&len,Makefile) != NO_EXIST)
    {   
        if (line == 2)
        {
            strtok(buf, "\n");
            strtok(buf, "\0");
            sprintf(VERSION,"%s",&buf[10]);
            _vprintf("VERSION=%s\n",VERSION);
        }
        else if (line == 3)
        {
            strtok(buf, "\n");
            strtok(buf, "\0");
            sprintf(PATCHLEVEL,".%s",&buf[13]);
            _vprintf("PATCHLEVEL=%s\n",PATCHLEVEL);
        }
        else if (line ==4)
        {
            strtok(buf, "\n");
            strtok(buf, "\0");
            sprintf(SUBLEVEL,".%s",&buf[11]);
            _vprintf("SUBLEVEL=%s\n",SUBLEVEL);
        }
        line++;
    }
    char fullver[128];
    strcat(VERSION,PATCHLEVEL);
    strcat(VERSION,SUBLEVEL);
    _vprintf("Kernel version = %s\n",VERSION);
    return VERSION;
}
...
get_proc_gz()
{
    ...
    FILE *inFile = access_file(g_conf_gz, "rb", SOFT_EXIST);
    FILE *outFile = access_file(g_oconfig, "w+", CREATE_NO_EXIST);

    if (inFile == NULL)
    {
        eprintf("Unable to access .gz files\n");
        exit(NO_EXIST);
    }
...
int main(int argc, char **argv)
{
    ...
    else if (_custom_config)
    {

        int ret = file_copy(g_iconfig, g_oconfig);
        if (ret == SUCCESS)
        {
            make_kernel();
        }
        ...
        {
            eprintf("Error accessing custom config\n");
        }
        else if (ret == NO_EXIST)
        {
            eprintf("Custom input file doesnt exist\n");
        }
    }
    else if (_extraction)
    {
        ...
        if (_custom_out)
        {
            printf("Do you want to make the kernel? (kerneldir: %s)", kernel_path);
            int choice = input();
            if (choice)
            {
                make_kernel();
    ...
    else if (_custom_kernel)
    {
        eprintf("Requires other arguments to be passed with custom kernel directory\n");
        return ERROR;
    }
    return SUCCESS;
}
