...
"-p \tPretend option. Dry-run without root access [WIP]\n"
...
Config file can be specified within interactive session [WIP]\n"
...
"11.\t5.19.10\n"
    ...
    if (menuconfig_choice){
        ...
        sprintf(menuconfigCommand,"cd %s && make olddefconfig && make menuconfig",kernel_path);
    ...
    if (opt){
        ...
        sprintf(command,
                "\
                cd %s && \
                make -j$(/bin/cat /proc/cpuinfo | grep processor | wc -l)  && \
                make modules_install -j$(/bin/cat /proc/cpuinfo | grep processor | wc -l)"
                ,kernel_path);

        printf("Command to generate %s\n",command);
        system(command);
        ...
        if (opt){
            ...
            sprintf(copyCommandbzImage,"cp %s/arch/x86_64/boot/bzImage\
                    /boot/vmlinuz-%s$(/bin/cat %s/.config | grep CONFIG_LOCALVERSION= | sed -r 's/^CONFIG_LOCALVERSION=//g' | tr -d '\"')"
                    ,kernel_path,kernel_version,kernel_path);
                        
            system(copyCommandbzImage);
            fflush(stdout);
        }

        printf("\nDo you want to regenerate grub-mkconfig? \n");
        int opt=input();
        if (opt){
            system("su -c \"grub-mkconfig -o /boot/grub/grub.cfg\"");
        }
    }
    exit(0);
}
...
"2. Gentoo Linux distribution kernel\n"
    ...
    switch(choice){
        case 1:
            ...
            file_copy("./arch-config",final_config_out_path);
            break;
        case 2:
            printf("Gentoo-dist-config selected\n ");
            file_copy("./gentoo-dist-config",final_config_out_path);
            break;
        default:
...
if (argc > 1){  
    ...
    while ((opt = getopt(argc,argv,"i:o:g:k:ehpa")) != EOF){
        ...
        case 'k':
            ...
            {
                printf("[x] Kernel directory doesnt exist\n");
                exit(1);
            }
            kernel_path=optarg;
            char path_to_kernel_config[1024];
            sprintf(path_to_kernel_config,"%s/.config",kernel_path);
            final_config_out_path=path_to_kernel_config;
            _custom_kernel++;
            continue;
        case 'a':
            printf("Configuring kernel with a distribution config\n");
            _use_dist_config++;
            continue;
...
void int_handler();
  ...
  if (_custom_config){
      ret = file_copy(input_opt_path,final_config_out_path);
      make_kernel();
  }
  else if (_extraction){
      ret = get_proc_gz();
